...
export class TaskService {
    constructor(taskId, taskTitle, taskText, isCompleted= false, isDeleted = false) {
     ...
     createTaskCard() {
        ...
        taskCard.setAttribute('data-task-id', this.taskId);
        if (this.isCompleted) taskCard.classList.add('_completed');
        if (this.isDeleted) taskCard.classList.add('_deleted');

        taskCard.innerHTML = `
        <div class="card__label">
            <h3 class="card__label_title">${this.taskTitle}</h3>
        ...
        <div class="card__body">
            <p class="card__body_text">${this.taskText}</p>
        </div>`;
